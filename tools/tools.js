...
module.exports = {
    ...
    convertUnixTimestampToDate: function (value) {
        let date = new Date(value * 1000);

        return date;
    },

    convertToStringDate: function (date) {
        return module.exports.Day[date.getDay()] + " " +
            module.exports.Month[date.getMonth()] + " " +
            date.getDate() + " " +
            date.getFullYear() + " " +
            date.getHours() + ":" +
            date.getMinutes() + ":" +
            date.getSeconds() + " " +
            "GMT" + ((date.getTimezoneOffset() <= 0) ? "+" : "-") + Math.abs(date.getTimezoneOffset() / 60);
    },
